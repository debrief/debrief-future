---
layout: future-post
title: "Planning: Browser-Based Extension Preview"
date: 2026-02-19
track: [momentum]
author: Ian
reading_time: 3
tags: [tracer-bullet, developer-experience, code-review]
excerpt: "PR preview environments so reviewers can test the VS Code extension in a browser -- no local install needed."
---
...
The preview container targets Heroku's 512MB RAM limit. code-server itself needs 200-300MB, which leaves headroom but not a lot. If the extension's memory footprint grows significantly -- say, loading larger STAC catalogs or rendering complex multi-layer maps -- we might hit that ceiling. Is Heroku's hobby tier the right fit long-term, or should we plan for a migration path to something with more resources?

The workspace opens with sample data pre-loaded, but we haven't settled on what the ideal "first thing a reviewer sees" experience looks like. A welcome document with testing instructions? The map view already showing tracks? The STAC explorer with the catalog expanded? The right answer probably depends on whether the reviewer is checking a map rendering change or a layers panel interaction, and we're not sure a single default covers both well.
