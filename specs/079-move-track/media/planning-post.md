---
layout: future-post
title: "Planning: Move Track Tool"
date: 2026-03-06
track: [momentum]
author: Ian
reading_time: 4
tags: [tool-spec, e03-demo, buffer-zone-analysis, move-track]
excerpt: "Specifying the second step in the E03 cascade: offsetting a vessel track by bearing and distance"
---
...
The tool accepts a compass bearing and distance. During the demo, there's also a map drag interaction -- the analyst grabs the moved track and drags it to a new position, which converts back into bearing and distance parameters. That drag-to-parameter conversion is a frontend concern, not part of this tool's contract.

Questions worth considering:

1. **Default distance**: The spec uses 5 nautical miles as the default range. That's reasonable for tactical-scale analysis in confined waters. For open-ocean scenarios, would a larger default (say 20nm) be more useful, or is it better to keep defaults small and let analysts increase?

2. **Bearing normalisation**: Bearings are normalised to [0, 360) via modulo. A bearing of -90 becomes 270 (West). Is there a case where analysts would expect negative bearings to be rejected rather than silently normalised?

3. **MultiLineString handling**: Compound tracks (multiple line segments in a single feature) have all segments translated together. Is there a scenario where you'd want to translate segments independently -- for example, offsetting only a specific leg of a multi-leg track?
