---
layout: future-post
title: "Planning: Schema-Validated GeoJSON Across All Services"
date: 2026-02-28
track: [momentum]
author: Ian
reading_time: 4
tags: [tracer-bullet, schema-validation, type-safety, tech-debt]
excerpt: "Enforcing shared schema validation at every GeoJSON boundary so field mismatches get caught at dev time, not in production"
---
...
- **Strictness vs. extension.** We are going with `extra = "forbid"` — any field not in the schema is rejected. This is safe but means every new property must be added to the schema first. If you work with GeoJSON features that carry ad-hoc metadata, does this create friction?

- **Validation performance.** We are targeting under 10ms per feature for tracks with up to 10,000 positions. For typical analysis workflows, is this overhead acceptable, or are there scenarios where batch validation of hundreds of features would become noticeable?

- **Frontend type coverage.** The TypeScript side already imports from `@debrief/schemas` in 30+ places, but a few workaround types (`SafeFeature`, custom `GeoJSONFeature` interfaces, `as any` casts) remain. Are there patterns in your own TypeScript GeoJSON handling that have worked better?
