---
layout: future-post
title: "Planning: Compound Track Model with Embedded Children"
date: 2026-02-08
track: [momentum]
author: Ian
reading_time: 4
tags: [tracer-bullet, schemas, geojson, track-model]
excerpt: "Sensors, TMA segments, and TUAs belong inside the track, not beside it"
---
...
The biggest design tension is between simplicity and structural enforcement. A single `SegmentMetadata` class with optional type-specific fields is easy to work with, but it means a `TRACK` segment technically accepts a `course` property (it would just be ignored). The alternative -- a class hierarchy with discriminated unions per segment type -- is more correct but considerably more complex in LinkML and in every consumer.

Some specific questions:

1. **Parallel array invariants.** The number of segments must match the number of LineStrings in the MultiLineString geometry, and each segment's positions count must match its coordinate count. LinkML can't express cross-field cardinality rules, so enforcement will be in Pydantic validators and golden fixtures. Is that sufficient, or should we pursue structural enforcement at the JSON Schema level?

2. **TUA positioning modes.** A TUA solution has either absolute positioning (lat/lon) or relative positioning (bearing/range from host), never both. We're documenting this as a logical constraint. Should we try to enforce mutual exclusivity structurally?

3. **Sensor contact identity.** Contacts are currently identified by position in the array. If tools need to reference specific contacts across sessions, should contacts carry stable IDs?
