---
layout: future-post
title: "Planning: Logical Result ID Registry"
date: 2026-02-13
track: [momentum]
author: Ian
reading_time: 4
tags: [tracer-bullet, results-visualization, provenance]
excerpt: "Building the lookup layer that connects tool outputs to their viewers"
---
...
- **Hydration strategy**: We reconstruct the registry from STAC metadata on every plot load
  rather than persisting registry state. This keeps things simple but means load time scales
  with the number of assets. For typical plots this should be negligible -- but are there
  use cases where a plot might accumulate hundreds of result artifacts?
- **Subscription granularity**: The current design supports per-result-ID subscriptions and
  a global "all changes" subscription. Is there a need for subscribing to groups of result
  IDs (e.g., "all results from this tool run")?
- **Naming conventions for logical result IDs**: Tools determine their own result IDs based
  on inputs. Should we establish a naming convention (e.g., `{tool_name}_{input_hash}`) or
  leave it to individual tool authors?
