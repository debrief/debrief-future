---
layout: future-post
title: "Planning: Snapshot Checkpoints with Doubly-Linked Chain"
date: 2026-02-09
track: [momentum]
author: Ian
reading_time: 3
tags: [tracer-bullet, provenance, prov-logging, snapshots, stac]
excerpt: "Clean-state checkpoints that let analysts navigate the full history of their analytical work."
---
...
- **Chain length in practice**: We're designing for chains up to ~50 snapshots per plot. Is that realistic for long exercises, or should we plan for more?
- **Snapshot naming**: We're using `plot-snap-{ISO-timestamp}.geojson` with hyphens replacing colons. Clear enough? Any filesystem edge cases we're missing?
- **Cross-snapshot deduplication**: When assembling a timeline across snapshot boundaries, we deduplicate on `activityId`. If an operation touched features that span two snapshots (unlikely but possible), should we handle that differently?
