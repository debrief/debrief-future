---
layout: future-post
title: "Planning: Saved Filter Configurations"
date: 2026-03-07
track: momentum
author: Ian
reading_time: 3
tags: [tracer-bullet, stac-browser, e08]
excerpt: "Analysts will be able to save, name, and reuse filter combinations in the STAC Browser — no more rebuilding the same filter set every session."
---
...
- Should saved filters be shareable between workspaces (export/import)? We've scoped this to workspace-local for now but CQL2 JSON makes export straightforward.
- Is 100 saved configurations per workspace a reasonable upper limit? We want to prevent unbounded growth without being restrictive.
- Would analysts want to organise saved filters into categories, or is a flat chronological list sufficient for the expected volume?
