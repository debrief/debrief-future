---
layout: future-post
title: "Planning: Sensor Bearing Lines on the Map"
date: 2026-04-10
track: [momentum]
author: Ian
reading_time: 3
tags: [sensor-pipeline, rendering, leaflet, mapview]
excerpt: "Adding bearing line rendering to MapView -- the moment sensor data becomes visible on the map."
---
...
- **Label density**: Legacy Debrief shows labels on every bearing line that has `show_label: true`. With hundreds of contacts, this creates visual clutter. Should we add automatic label thinning based on zoom level?
- **Sensor arc priority**: The SENSORARC format creates coverage fan shapes. The schema for this doesn't exist yet -- should we defer arc rendering to a follow-up, or define a minimal arc representation now?
- **Selection interaction**: When a user clicks a bearing line, what should happen? Select the parent track? Select the individual sensor? Select the specific contact? Legacy Debrief didn't support clicking bearing lines at all.
