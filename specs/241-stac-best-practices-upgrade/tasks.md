...
- [x] T022 Add `ITEM_ASSETS_TEMPLATE` module-level constant to `collection.py` containing the five logical asset declarations (`features`, `thumbnail`, `overview`, `source`, `scene-thumbnail`) per data-model.md. `services/stac/src/debrief_stac/collection.py`
- [x] T023 Update `collection.rebuild_collection_summaries()` to emit `stac_version: "1.1.0"`, set `license` to `"other"` if absent (or normalise from the deprecated `"proprietary"` value), set `providers` to `DEFAULT_PROVIDERS` if absent, write the `item_assets` block from `ITEM_ASSETS_TEMPLATE`, and add a `links[]` entry with `rel: "license"` when `license == "other"`. Existing `summaries` computation unchanged. `services/stac/src/debrief_stac/collection.py`
- [x] T024 [test] Extend `test_collection.py` to assert the rebuilt Collection validates against `contracts/collection-shape.schema.json` AND the vendored STAC 1.1 Collection Schema. Cover FR-010, FR-011, FR-012, FR-014. `services/stac/tests/test_collection.py`
- [x] T025 [P][test] Write a regression test for FR-013: the Collection's `summaries` contents (debrief:tags / debrief:feature_tags / debrief:platforms aggregations) are byte-identical to the 1.0 baseline for the same Item set. `services/stac/tests/test_collection.py`
- [x] T026 [P][test] Write a test asserting that when `license == "other"`, the Collection has exactly one `links[]` entry with `rel: "license"`; when `license` is an SPDX expression, no such link is required. `services/stac/tests/test_collection.py`
...
- [x] T042 Create `scripts/upgrade-catalog-to-stac-1.1.py` implementing the regeneration pipeline per `contracts/factory-api.md`. `scripts/upgrade-catalog-to-stac-1.1.py`
- [x] T043 Make T042 idempotent: every field-update step checks current state before mutation; `properties.updated` is only refreshed when other content actually changed. Verified by `test_regen.py::test_regen_idempotent` and `evidence/regeneration-output.txt`. `scripts/upgrade-catalog-to-stac-1.1.py`
- [x] T044 Ran `uv run python scripts/upgrade-catalog-to-stac-1.1.py` against `preview/workspace/samples/local-store/` — all 73 items + catalog validate against vendored STAC 1.1 schemas. Stdout captured in `evidence/regeneration-output.txt`.
- [x] T045 Renames staged via `git mv`. Note: when both passes happen in a single commit, `git status --short` displays mod+add+del because the canonical `thumbnail.png` filename hosts different content before/after the rename pair; subsequent `git log --follow overview.png` will trace through the rename via git's content-based heuristic at log time.
- [x] T046 [test] `services/stac/tests/test_regen.py` — 4 tests: spec-241 shape produced; idempotent on a 3-item fixture (FR-019); halts on corrupt input (FR-020); regenerated items satisfy existing invariants (FR-021).
- [x] T047 [test] `test_stac_validation.py` adds `test_sample_catalog_items_validate_against_stac_1_1` and `test_sample_catalog_root_validates_against_stac_1_1` — walk every item + the catalog (SC-001).
- [x] T048 [P] Sample item shape captured in `evidence/sample-item-diff.md` (`core--boat1` canonical example).
- [x] T049 [P] Sample catalog shape captured in `evidence/sample-collection-diff.md`.
...
- [x] T050 Created `StacBrowserPage.ts` — page object with gotoCatalog/waitForCollectionLanding/clickFirstItem/expandAssets/expectVisibleText/expectThumbnailRendered. `apps/web-shell/playwright/pages/StacBrowserPage.ts`
- [x] T051 Created spec with `beforeAll` spawning two `http-server` instances (catalog on :4080, stac-browser dist on :8080), driving Collection → Item → Assets, capturing three screenshots to `specs/241-stac-best-practices-upgrade/evidence/`. `afterAll` stops both servers. `apps/web-shell/playwright/tests/stac-browser-interop.spec.ts`
- [x] T052 Implemented FR-024 + FR-025 assertions: collection landing visible, thumbnail rendered, item card clickable; console.error filtering tolerates debrief:* warnings + external resource fetch failures (OSM tiles, axios network errors against unreachable upstream URLs); rejects schema/JSON/JS errors that originate in OUR catalog.
- [x] T053 60s timeout via `test.setTimeout(60_000)` on the describe block.
- [x] T054 Spec auto-discovered by `apps/web-shell/playwright/playwright.config.ts`'s default `testDir: './tests'` glob.
- [x] T055 [test] 3/3 consecutive runs pass with zero retries (SC-003). Each run completes in ~5.9s well under the 60s budget.
- [x] T056 All three screenshots present in `specs/241-stac-best-practices-upgrade/evidence/`: `stac-browser-collection.png`, `stac-browser-item.png`, `stac-browser-assets.png`.
