---
layout: future-post
title: "Planning: REP Sensor Import"
date: 2026-04-10
track: [momentum]
author: Ian
reading_time: 3
tags: [sensor-data, rep-parser, debrief-io, e07-sensor-pipeline]
excerpt: "Parsing legacy sensor contact lines from REP files and embedding them directly into track features"
---
...
- The current schema (pre-#116) lacks boolean presence flags (`has_bearing`, `has_ambiguous`, `has_frequency`). We're including these in the output dicts now, anticipating #116. Does that forward-compatibility approach seem right, or should we wait for the schema to land first?
- SENSORARC lines produce standalone annotation features. Should they instead be embedded in the track like sensor contacts, or is the annotation approach correct for coverage zones?
- The spec says range in SENSOR v1/v2/v3 lines is always in yards. Are there real-world datasets where range might already be in metres?
