---
layout: future-post
title: "Planning: Tabular Results Panel comes to the VS Code extension"
date: 2026-04-07
track: [momentum]
author: Ian
reading_time: 3
tags: [tracer-bullet, vscode-extension, tabular-results, provenance]
excerpt: "Tool outputs in the VS Code extension are about to grow a Results panel — tables, charts, save, and round-trip provenance."
---
...
- **Open behaviour.** Round-tripping CSV through the table parser feels right for symmetry, but is the first thing you'd want when you open a saved result actually the original view, or VS Code's text editor with diff/git history? If the latter, we'd flip the default.
- **Cleanup semantics.** Right now, closing a tab does *not* delete the orphan ToolRunEvent — only closing the plot does. Is that the right granularity? Or should "I'm closing this tab, I don't want this run" be an explicit way to drop a failed experiment from the audit trail?
- **Panel placement.** Bottom panel area (current plan) vs. secondary side bar — which feels more natural during an analysis session?

The full spec, research notes, and contracts live in [`specs/178-vscode-tabular-results/`](https://github.com/debrief/debrief-future/tree/claude/vscode-tabular-results-CFFcS/specs/178-vscode-tabular-results).
