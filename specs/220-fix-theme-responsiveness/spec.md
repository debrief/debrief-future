...
- Q: How should the theme variant model represent high-contrast (FR-008)? → A: Flat union `'light' | 'dark' | 'high-contrast-light' | 'high-contrast-dark' | 'system'`, mirroring VS Code's body-class taxonomy. The legacy `'vscode'` variant is retired.
...
A developer working on a Debrief component opens Storybook and can preview the component in each of the four supported variants (light, dark, high-contrast-light, high-contrast-dark) from a single interface. All variants are visually correct and ready to screenshot for review.
...
**Independent Test**: Open Storybook, switch the global theme selector through "Light", "Dark", "High Contrast Light", and "High Contrast Dark", and verify that all stories render with visibly distinct colours in each variant with no hardcoded colours leaking through.
...
2. **Given** the Storybook theme is set to "High Contrast Dark", **When** a component is rendered, **Then** it uses the high-contrast-dark colour tokens (heavier borders, distinct focus rings) rather than the generic dark palette.
...
- **FR-005**: The Storybook development environment MUST provide a theme switcher exposing all four explicit variants (`light`, `dark`, `high-contrast-light`, `high-contrast-dark`) plus the `system` pass-through, so developers can verify theme correctness without launching the VS Code extension.
...
- **FR-008**: High-contrast accessibility themes MUST be treated as a first-class variant, not mapped to the generic dark theme. The shared theme variant model MUST distinguish high-contrast variants from regular dark/light variants so a component can adjust contrast-sensitive styling (focus rings, borders, selection indicators) without inferring it from colour values. The variant model MUST be a flat union: `'light' | 'dark' | 'high-contrast-light' | 'high-contrast-dark' | 'system'`, mirroring VS Code's own body-class taxonomy (`vscode-light`, `vscode-dark`, `vscode-high-contrast-light`, `vscode-high-contrast`). The legacy `'vscode'` variant value MUST be retired — when running inside a VS Code webview the variant is one of the four explicit values resolved from VS Code's body class.
...
- **Theme Variant**: A flat enumeration `'light' | 'dark' | 'high-contrast-light' | 'high-contrast-dark' | 'system'`, matching VS Code's body-class taxonomy. The legacy `'vscode'` value is retired; inside a VS Code webview the variant resolves to one of the four explicit values. `'system'` follows the OS `prefers-color-scheme` and `prefers-contrast` signals when used outside a VS Code webview.
...
- **SC-004**: Storybook stories for all components render correctly in all four theme variants (light, dark, high-contrast-light, high-contrast-dark) with no component showing a purely white or black background that doesn't match the selected variant.
...
- **SC-009**: Storybook stories using `var(--vscode-...)` colour variables render with the correct theme-derived colour in each of the four explicit variants (light, dark, high-contrast-light, high-contrast-dark), verified by visual snapshots that compare each variant against the hardcoded-fallback rendering.
