...
- **Updated post-`/speckit.review`** (2026-05-08): scope reduced — `StacItem` typing dropped (deferred to a follow-up backlog item; LinkML's `gen-typescript` strips the `debrief:` JSON-key prefix, so naive intersection wouldn't deliver the promised flow); `PropertiesProvenanceEntry` migration adopts a hybrid intersection that re-exports the LinkML-generated type and statically narrows `tool`/`method`/`source` to literal types in `@debrief/components/PropertiesPanel/provenanceTypes.ts`. SC-007 added to gate the drift check on verified generator determinism. Stories renumbered (was 3 → now 2).
