...
- [ ] T022 Run the full pre-push CI pipeline (`task verify`) and confirm lint, typecheck, unit tests, schema round-trip, and the new Playwright spec all pass. If any step fails, fix the underlying issue (do NOT skip hooks). This is the gate that guarantees Article II / VI / XV compliance before evidence capture
