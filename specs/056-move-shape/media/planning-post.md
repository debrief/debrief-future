---
layout: future-post
title: "Planning: Move Shape Tool Spec"
date: 2026-02-10
track: [momentum]
author: Ian
reading_time: 3
tags: [tool-spec, shape-manipulation, great-circle, annotations]
excerpt: "Specifying how annotation shapes translate across the Earth's surface using great-circle math"
---
...
Two golden I/O pairs cover the complex cases -- a circle (polygon vertices plus `center` property) and a vector (`origin` update, `range`/`bearing` preservation). The simpler annotation kinds (rectangle, line, text) are subsets of that logic.

Questions worth considering:

1. **Is spherical adequate?** For annotation repositioning at 5-50 km distances, the spherical Vincenty formula introduces sub-metre error compared to ellipsoidal. That seems acceptable for shape annotations, but if anyone has use cases involving much larger distances, the spec might need a precision tier.

2. **Edge case coverage in golden examples**: Should we add a third example for antimeridian crossing, or is documenting the normalisation formula in the algorithm sufficient?

3. **Provenance label format**: The spec produces labels like "Translated 3 shape(s) 5 km bearing 090". Is there a format that would be more useful when reviewing analysis logs?
