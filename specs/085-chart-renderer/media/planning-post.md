---
layout: future-post
title: "Planning: Chart Renderer and Dataset Transformer"
date: 2026-02-13
track: [momentum]
author: Ian
reading_time: 3
tags: [tracer-bullet, shared-components, e04-results-visualization, vega-lite]
excerpt: "Building the foundation for results visualization -- a shared chart component and the transformer that sits between tools and rendering"
---
...
The dataset envelope schema defines axis types as Vega-Lite encoding types (`nominal`, `ordinal`, `quantitative`, `temporal`). These map cleanly to the rendering library, but they also leak an implementation detail into the tool output format. Is that an acceptable trade-off, or should tools use domain-specific labels (like `categorical` and `numeric`) that the transformer maps internally?

We're launching with two dataset types. Which tool outputs would you most want to see visualized next -- bearing-time plots, frequency analysis, or something else entirely?
