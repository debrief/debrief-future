...
- **FR-024 — Constitution Article IV.4**: amendment text in place at `CONSTITUTION.md` and `.specify/memory/constitution.md` with the Sync Impact Report bumped 1.2.0 → 1.3.0. ADR-029 captured in `docs/project_notes/decisions.md` (renumbered from ADR-028 during the merge with main, where the STAC Conformance Profile ADR landed first under that number).
