---
title: "Building Web-shell STAC writes — IndexedDB persistence and a unified writer interface"
date: 2026-05-02
authors:
  - claude
spec: 236-web-shell-stac-writes
hook_type: mermaid
tags:
  - architecture
  - constitution
...
The before/after visual contrast is the headline of this work:

- **after-no-badge.png** — `Persistence demo` storyboard with a captured scene in the rail, no warning. With healthy IndexedDB the writer persists the capture and the badge stays hidden.
- **before-session-only-badge.png** — same content, with `globalThis.indexedDB` stubbed to undefined. The yellow badge appears at the top of the rail with the reason-specific message: "⚠ Session-only — captures persist only for this tab. Browser persistence unavailable."
