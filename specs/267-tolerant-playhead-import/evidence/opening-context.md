...
- **Never silent.** Article I.3 says users must always know the state of their data, so every clamp surfaces a non-blocking notification and is recorded in the plot's provenance log on next save. The relaxation makes the data issue loud while still letting you work.
