...
- **Multiple plots opened in a batch / session restore**: not a current path — both hosts load plots one at a time, so at most one clamp/notification per load. (If a batch-restore path is added later, coalescing is a deferred backlog item — see /speckit.review.)
...
- **FR-006** *(removed after /speckit.review — deferred to backlog)*: Notification coalescing for multi-clamp loads was dropped. Plot loads are **per-plot** in both hosts (VS Code `openPlot`, web-shell `App.tsx`), so a single load produces at most one clamp and one notification — there is no batch-restore path that could stack notifications. The notification MUST be non-blocking/non-modal regardless (covered by FR-003). If a batch/session-restore path is ever added, coalescing is captured as a backlog item (review deferred-items).
- **FR-007** *(revised after spec-261 merged)*: The clamp MUST leave a durable-until-healed record. spec-261 ships view-state `SystemState` features **without a `provenance` field** (they are deliberately lean — 261 FR-013; the temporal Zod schema is `.strict()` with no provenance slot), so the clamp is NOT recorded as a per-feature provenance `LogEntry`. Instead, the clamp MUST surface a non-blocking notification on **every** load of the orphaned plot (FR-003) — i.e. the recovery is idempotent and re-announced each time — until the analyst saves, which persists the in-window value (FR-008) and ends the condition. Article I.3 ("users must always know the state of their data") is satisfied by the repeating notification; Article III.1 is not engaged because a load-time UI-playhead recovery is not an analytical transformation and 261 already exempts view-state markers from provenance.
- **FR-008**: The clamp MUST be applied in-memory only on load; it MUST NOT by itself mark the plot dirty or trigger an automatic save (preserving spec-261 FR-017 — playhead changes persist only via explicit save). On the next explicit save, the now-in-window playhead value is written through spec-261's existing `write.ts` path, healing the stored value (see FR-001 acceptance scenario 4).
...
- **SC-004**: Opening a plot with an orphaned playhead requires zero blocking dismissals to proceed — the notification is non-blocking/non-modal (per-plot load; multi-plot coalescing deferred to backlog per /speckit.review).
...
1. **Spec-261 is MERGED (2026-05-29) and provides the substrate.** This feature amends spec-261's shipped code: `checkTemporalCrossField` (`validate.ts`), `readSystemStateFromFeatureCollection` (`read.ts` — the throw site), and `hydrateStoreFromFeatures` (`store-bridge.ts` — the both-host load entry). The earlier assumption that 261 was unimplemented is resolved; the real surfaces differ from 261's planned contract (no `reconcile.ts`, no `persistence/load.ts`, and — critically — no `provenance` on view-state markers, which forced the FR-007 revision above). See `contracts/system-state-helper-delta.md`.
...
- **Hard dependency on spec-261** (`261-session-state-systemstate`) — **MERGED**: the `current_time` field, the shared SystemState helper (`read.ts`/`store-bridge.ts`/`validate.ts`/`write.ts`), the cross-field check `checkTemporalCrossField` + `SystemStateLoadError`, and the no-dirty-on-playhead contract (261 FR-017). This feature is a direct amendment to 261's strict-on-import policy for the `current_time` sub-case. **Note**: 261 shipped view-state markers *without* provenance (261 FR-013) — see the revised FR-007.
