...
- **FR-008**: The live transport MUST map provider-side failures into #188's existing `GenerationError` shape with reason identifiers that distinguish, at minimum: authentication/authorization failure, rate-limit / quota failure, network/transport failure, timeout, malformed-response, and provider-returned-error. The demo UI MUST display a message chosen from these classes rather than a raw stack trace or HTTP status code.
...
- **SC-005**: Each of the 5 failure classes called out in FR-008 (auth, rate-limit, network, timeout, malformed, provider-error) surfaces a distinct, user-readable message in the demo UI — verifiable by injecting each condition via the test stub.
- **SC-006**: No credential value appears in any file inside the deployed demo bundle, verifiable by a repository scan that looks for the configured credential signature in built artefacts.
