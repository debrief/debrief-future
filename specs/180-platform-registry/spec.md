...
3. **Given** a new platform is added to the YAML file under an existing class, **When** the registry is reloaded and enumeration is called, **Then** the new platform appears in the list with correctly derived metadata.
...
1. **Given** the same registry YAML file, **When** both Python and TypeScript loaders resolve `platform_id = "MASON"`, **Then** both return identical values for all fields (name, nationality, vessel_class, vessel_type, vessel_role, domain).
2. **Given** the same registry YAML file, **When** both loaders enumerate all platforms, **Then** both return the same count and the same set of platform IDs.
...
- What happens when the registry YAML file is missing or unreadable? The loader raises/throws a clear error at load time rather than silently returning empty results.
...
- **FR-001**: The system MUST provide a single YAML registry file at `shared/data/platform-registry.yaml` that defines the vessel class hierarchy as a tree, with vessel classes as interior nodes and individual platforms as leaf entries.
- **FR-002**: Each platform leaf entry MUST include at minimum: a platform ID (the YAML key), a human-readable `name`, and a `nationality` (ISO 3166-1 alpha-2 country code).
...
- **FR-006**: The system MUST provide a Python loader that reads the YAML file and exposes: (a) single-platform resolution by ID, (b) enumeration of all platforms, and (c) tree traversal by class path.
- **FR-007**: The system MUST provide a TypeScript loader that reads the YAML file (or a build-time JSON conversion of it) and exposes the same three capabilities as the Python loader.
...
- **FR-010**: The loaders MUST validate the registry at load time and report errors for: missing/unreadable file, duplicate platform IDs across branches, and malformed platform entries (missing required fields).
...
- **SC-004**: Adding a new platform to the registry requires editing only the YAML file — no code changes are needed for the new platform to be resolvable.
...
- The YAML file format uses the tree structure documented in the E10 epic: `vessel_classes` as the root key, with `surface` and `subsurface` as top-level domains.
...
- Runtime registry modification (the file is static; changes require editing the YAML)
