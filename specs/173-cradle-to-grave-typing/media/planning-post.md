---
layout: future-post
title: "Planning: Cradle-to-Grave Typing"
date: 2026-03-24
track: [momentum]
author: Ian
reading_time: 4
tags: [tracer-bullet, type-safety, linkml, pydantic, typescript, schema-first]
excerpt: "Replacing ~150 places where dict[str, Any] and Record<string, unknown> let invalid property access go undetected."
---
...
The migration order matters. We're planning to start with the Python executor boundary (fail-fast validation on tool output) and then work outward to individual tools, then TypeScript consumers. The alternative is to start on the TypeScript side, where the type guards are more mature and the escape hatch is a cleaner target. If you've worked with either side of the codebase and have a view on which order reduces risk, that's worth hearing.

The `featureProps.ts` elimination touches 10 files, each of which needs a type guard added. In most cases the guard is obvious (`setTrackColor` operates on tracks, `moveShape` operates on shapes). But `calcService.ts` and `mapPanel.ts` handle multiple feature kinds -- the narrowing logic there will be more involved. If anyone has dealt with discriminated union narrowing across polymorphic dispatch in TypeScript and has patterns that worked well, I'd welcome pointers.

The session-state type generation is the biggest open question. Adding session-state to the TypeScript generator means the Zustand store types come from LinkML rather than being hand-written. That's architecturally clean, but it means schema changes ripple into the store layer. We think this is a feature, not a bug -- but it's a real change in how the session-state package evolves.
