...
- **#263 — Storyboard Time-Range Scenes** (**shipped 2026-05-19, commit 38bce0d**): defines the `time_range` and `viewport_end` shape and the simultaneous viewport+slider playback contract that this feature inherits. Specifically supplies (a) the optional `time_range` and `viewport_end` slots on `SceneProperties`, (b) the `flavourCheck()` XOR validator and `isTimeRangeScene()` predicate in `@debrief/components/storyboard`, and (c) the host-agnostic `runTimeRangeTween` primitive + `defaultScheduler` in `@debrief/components/storyboardPlayback`. The briefing SPA reuses all three verbatim. Implementation of this feature is therefore no longer blocked.
