---
layout: future-post
title: "Planning: Consolidating bounds utilities (and fixing a silent-miss bug we found on the way)"
date: 2026-04-20
track: [momentum]
author: Ian
reading_time: 4
tags: [tracer-bullet, tech-debt, vscode-extension, shared-utils]
excerpt: "Two copies of calculateBounds had drifted. The review gate caught a third problem next door — a silent-miss zoom bug — and the PR got bigger for the right reasons."
---
...
A drift-prevention rule would make the "exactly one" guarantee durable. Something that fails CI if `apps/*/src/utils/bounds.ts` reappears, or more generally if any `apps/*` file re-exports a symbol already in `shared/utils`. Lint rule? Grep in CI? Worth it, or too cute? Open question.

And `calculateBounds` is unlikely to be the only near-duplicate in the monorepo. If you've spotted others in passing, I'd like to know which so we can queue them up.
