...
- "Mobile chunk and vice-versa" in the original ticket is interpreted as the primary-direction outcome — desktop never ships the mobile subtree. Wrapping the desktop tree in a symmetric `React.lazy()` boundary to also remove desktop code from mobile transfers is **out of scope** for this iteration. If subsequent measurement shows mobile transfer dominated by desktop-only modules, that work can be a follow-up.
