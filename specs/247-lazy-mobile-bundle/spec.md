...
- **Scope is mobile-only.** This iteration lazy-loads the mobile subtree only; the desktop component tree remains eagerly imported by the entry chunk. The "and vice-versa" phrasing in the original ticket is **explicitly out of scope** here — wrapping the desktop tree in a symmetric `React.lazy()` boundary to also remove desktop code from mobile transfers is deferred. Trigger for revisiting: subsequent mobile-side bundle measurement showing mobile transfer dominated by desktop-only modules. Tracked as backlog item #249.
