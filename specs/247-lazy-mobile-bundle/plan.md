...
**Constraints**:
- The 1024px breakpoint must remain a single source of truth aligned with the existing `MOBILE_BREAKPOINT_MAX = 1023` constant (FR-008). **Implementation requirement**: the lazy boundary's mobile/desktop decision MUST consume the existing `useIsMobile` hook directly (subpath imported from `@debrief/components`); no parallel `matchMedia` call, ad-hoc `window.innerWidth` read, or duplicated breakpoint constant may be introduced. This consolidates the breakpoint inside this PR rather than deferring it.
...
| **VIII. Documentation** | VIII.1 Specs before code; VIII.3 ADRs for significant choices | ✅ Pass | Spec written first (#247). The bundle-guard contract change (entry-chunk-only measurement vs. summed-JS) is a permanent, CI-gating cross-feature contract — promoted to a dedicated ADR per review decision 3A. ADR will be authored alongside the implementation and tracked as backlog item #253 if not landed in this PR. The chunk-failure error boundary is localised to this feature's surface and does not warrant its own ADR. |
