...
| **VIII. Documentation** | VIII.1 Specs before code; VIII.3 ADRs for significant choices | ✅ Pass | Spec written first (#247). The bundle-guard contract change (entry-chunk-only measurement vs. summed-JS) is a permanent, CI-gating cross-feature contract — promoted to a dedicated ADR per review decision 3A. ADR will be authored alongside the implementation and tracked as backlog item #250 if not landed in this PR. The chunk-failure error boundary is localised to this feature's surface and does not warrant its own ADR. |
