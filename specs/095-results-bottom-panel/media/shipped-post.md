...
![Results panel showing three tabs — a chart, an image, and a GeoJSON file](results-panel-three-tabs.png)
*Three result types open simultaneously: a Vega-Lite chart (left tab), a satellite image with pan/zoom (center), and GeoJSON metadata (right)*

![Context menu on catalog tree item with "Open Result" command](context-menu-open-result.png)
*Right-click any result in the catalog tree to open it directly — no need to wait for tool execution*

![Tab disambiguation when the same result belongs to multiple plots](disambiguated-tabs.png)
*When "range_rings.json" exists in both Plot A and Plot B, tabs show "Range Analysis (Plot A)" and "Range Analysis (Plot B)"*

![Auto-open behavior after tool execution](auto-open-after-tool.png)
*Panel appears automatically when a tool completes, focusing the newest result*
...
→ [See the specification](/home/user/debrief-future/specs/095-results-bottom-panel/spec.md)
