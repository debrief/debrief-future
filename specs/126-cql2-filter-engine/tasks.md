...
- [ ] T001 Install cql2-filters-parser dependency in shared/components `shared/components/package.json`
- [ ] T002 Create filter-engine module directory and barrel export `shared/components/src/filter-engine/index.ts`
- [ ] T003 [P] Create types module from contract definitions `shared/components/src/filter-engine/types.ts`
- [ ] T004 [P] Add taxonomy JSON adapter to convert fixture format to VesselTaxonomyNode[] `shared/components/src/filter-engine/taxonomy.ts`
- [ ] T005 Add filter-engine subpath export to package.json `shared/components/package.json`
...
- [ ] T006 Implement taxonomy descendant expansion: pre-compute nodeId → Set\<fullPath\> map `shared/components/src/filter-engine/taxonomy.ts`
- [ ] T007 [test] Write taxonomy expansion tests (parent/leaf/unknown/empty tree) `shared/components/src/filter-engine/__tests__/taxonomy.test.ts`
- [ ] T008 Implement per-filter-type matcher functions (all 9 types) `shared/components/src/filter-engine/matchers.ts`
- [ ] T009 [test] Write matcher tests for each filter type `shared/components/src/filter-engine/__tests__/matchers.test.ts`

**Matcher details** (T008):
- `vessel-class`: lookup expanded paths from taxonomy map, check item.vesselClasses intersection
- `plot-tag`: case-insensitive array contains on item.tags
- `feature-tag`: case-insensitive array contains on item.featureTags
- `author`: case-insensitive exact match on item.author
- `duration`: compute ms from startDatetime/endDatetime, compare to bucket threshold
- `title`: case-insensitive substring match on item.title
- `track-name`: case-insensitive array contains on item.trackNames
- `nationality`: case-insensitive array contains on item.nationalities
- `collection`: exact match on item.collection
...
- [ ] T010 Implement createFilterEngine factory and filter/matches methods (AND-only path) `shared/components/src/filter-engine/engine.ts`
- [ ] T011 [test] Write engine AND logic tests (single predicate, multi-predicate, empty filter, no matches) `shared/components/src/filter-engine/__tests__/engine.test.ts`
...
- [ ] T012 Extend engine.filter to handle orGroups (OR within AND evaluation) `shared/components/src/filter-engine/engine.ts`
- [ ] T013 [test] Write engine OR logic tests (OR-only, mixed AND+OR, multi OR groups, single-predicate OR) `shared/components/src/filter-engine/__tests__/engine.test.ts`
...
- [ ] T014 Create test fixture helper to load mock items from #125 fixtures as StacBrowserItem[] `shared/components/src/filter-engine/__tests__/fixtures.ts`
- [ ] T015 [test] Write integration tests against real #125 fixtures (all 9 filter types, hierarchical vessel-class, duration buckets, title search) `shared/components/src/filter-engine/__tests__/integration.test.ts`
...
- [ ] T016 Implement toCql2Json serialisation (AND, OR, mixed, empty, all operator mappings) `shared/components/src/filter-engine/cql2-json.ts`
- [ ] T017 Wire toCql2Json into FilterEngine interface `shared/components/src/filter-engine/engine.ts`
- [ ] T018 [test] Write CQL2 JSON serialisation tests (AND-only, OR-only, mixed, empty, array-contains, LIKE) `shared/components/src/filter-engine/__tests__/cql2-json.test.ts`
...
- [ ] T019 Verify all tests pass and run full test suite `shared/components/`
- [ ] T020 Update barrel export in filter-engine/index.ts with all public API `shared/components/src/filter-engine/index.ts`

### Evidence Collection (REQUIRED)

- [ ] T021 Capture test results using template (.specify/templates/evidence/test-summary-template.md) in `specs/126-cql2-filter-engine/evidence/test-summary.md`
- [ ] T022 Create usage demonstration in `specs/126-cql2-filter-engine/evidence/usage-example.md`
- [ ] T023 [P] Capture sample filter inputs and outputs in `specs/126-cql2-filter-engine/evidence/filter-output-samples.json`
