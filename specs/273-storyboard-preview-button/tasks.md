...
| screenshots/vscode-{storyboard-capture,storyboard-panel,preview-trigger,preview-playback,preview-scene-3,preview-present}.png | Desktop (VS Code) surface: real capture → populated panel (real thumbnails) → Preview trigger → loopback-launched renderer playback (code-server E2E) | After web-shell evidence (desktop follow-up) |
...
- [x] T039a [test] Add the VS Code desktop-surface evidence E2E `tests/e2e/test-storyboard-preview-vscode.spec.ts` (real code-server + real extension via `cloud-e2e-setup.sh`; CI-skipped, gated on `E2E_REFRESH_VSCODE_SCREENSHOTS=1`). Test 1 drives the genuine capture pipeline (Capture → inline naming → real map-thumbnail Scenes) for `vscode-storyboard-capture.png` + `vscode-storyboard-panel.png`; Test 2 seeds a 3-scene storyboard, clicks Preview, detects the loopback `BriefingPreviewServer` port and captures the renderer it serves for `vscode-preview-{trigger,playback,scene-3,present}.png`. Run: `bash tests/e2e/scripts/cloud-e2e-setup.sh --setup-only` then the gated `pnpm exec playwright test --config tests/e2e/playwright.config.ts tests/e2e/test-storyboard-preview-vscode.spec.ts`.
