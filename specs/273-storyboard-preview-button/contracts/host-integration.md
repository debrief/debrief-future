...
- **C-B4**: The extension opens the system browser at `await asExternalUri(http://127.0.0.1:<port>/?features=features.geojson)` via `openExternal`. The `features` value is **relative** so it resolves against the renderer's document URL and survives a proxy path-prefix (e.g. code-server's `/proxy/<port>/`); an absolute `/features.geojson` would escape that prefix. Because code-server's `asExternalUri` rewrite *also drops the query string*, the server additionally injects the features location into the served `index.html` as `window.__BRIEFING_PREVIEW_FEATURES__`; the renderer reads it (via `resolveFeaturesUrl`) when `?features` is absent, so the active storyboard loads even when the query is lost.
