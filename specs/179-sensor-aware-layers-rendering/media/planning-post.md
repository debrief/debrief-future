---
layout: future-post
title: "Planning: Sensor-Aware Track Rendering in the Layers Panel"
date: 2026-04-09
track: [momentum]
author: Ian
reading_time: 4
tags: [tracer-bullet, layers-panel, sensor-data, e07]
excerpt: "Making sensor contacts visible in the Layers panel so analysts can verify loaded data at a glance."
---
...
Three things we'd genuinely like other eyes on before we start building:

- Is the group-row-as-selection-unit the right semantic for downstream map and chart integrations? We picked it for simplicity, but a collection-ID selection is a new concept for consumers — worth checking that's the shape you'd want if you were wiring a map highlight.
- The zero-padding change on courses will refresh every position row in every track across the panel. We think consistency with bearings is the right call, but it's a visible change that existing users will notice immediately. Any reason to scope it tighter?
- Info-icon on contact rows, sensor-metadata popover deferred. Comfortable with that split, or does it feel like the two should ship together?

→ [Spec and contracts on the branch](https://github.com/debrief/debrief-future/tree/179-sensor-aware-layers-rendering/specs/179-sensor-aware-layers-rendering)
