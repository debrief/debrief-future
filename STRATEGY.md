...
| VS Code E2E webview reliability research sprint (BACKLOG #142, subsumes #135) | Wrong phase: high-complexity research sprint (A:2) on openvscode-server internals while demo-critical items (#079, #081, #084, #132) are waiting. Web-shell E2E already covers all React components. Hybrid approach (candidate F from idea file) adopted as pragmatic strategy: VS Code E2E tests extension mechanics, web-shell E2E tests webview DOM. Scored V5/M3/A2=10 but strategic override to V3 — testing fidelity gap, not a blocker | Full VS Code webview fidelity needed (e.g., integration testing beyond what web-shell covers) or openvscode-server fixes the `resolveWebviewView` lifecycle upstream |
...
| 2026-03-18 | Parked #142 VS Code E2E webview reliability (subsumes #135) | Strategic override: prioritizer scored V5 but this is a testing fidelity gap, not a tracer bullet blocker. A:2 means high human attention cost during a phase with multiple demo-critical items waiting. The hybrid approach (candidate F) is the pragmatic choice now — web-shell E2E covers React components, VS Code E2E covers extension mechanics. Both are working. The ~50 skipped tests are aspirational coverage, not a regression. Revisit if openvscode-server fixes `resolveWebviewView` upstream or if integration boundary bugs emerge that web-shell cannot catch |
