...
| 2026-05-20 | (this PR)  | **#223 STAC catalog cluster resolved** — see [spec 223](../specs/223-linkml-stac-catalog/spec.md). §3.1 `cross-domain-hand-typed` rows attributed to #223 dropped from **8 → 0** (5 audit-flagged sites — `StacItem`×3 + `StacCatalog`×2 — plus 3 additional rows from `@debrief/stac-writer.StacItem`, `@debrief/stac-writer.StacAsset`, and `apps/vscode/src/types/stac.ts:StacAsset`). §3.2 `StacItem` (3 members) and `StacCatalog` (2 members) drift clusters dropped from **5 → 0**. 11 LinkML classes + 1 permissible-values enum added under `shared/schemas/src/linkml/stac.yaml` (`StacItem`, `StacCatalog`, `StacCollection`, `StacLink`, `StacAsset`, `StacItemAssetDefinition`, `StacExtent`, `StacSpatialExtent`, `StacTemporalExtent`, `StacSummaries`, `StacProvider`, plus `StacItemProperties` inline mixin and `StacTypeEnum`). TS-only `StacCatalogOrCollection` discriminated-union alias at `shared/schemas/src/typescript/aliases/stac-unions.ts` is R4-import-schema-rooted. Three open-record classes (`StacItemProperties`, `StacAsset`, `StacSummaries`, `StacItemAssetDefinition`) carry the Article XV.2 `extra='allow'` exception to preserve STAC's `<namespace>:<key>` extension convention. The `@debrief/stac-writer` package's local `StacItem` and `StacAsset` declarations were also deleted and replaced with `@debrief/schemas` re-exports, closing spec A-009 (the JSON-projection cast at `apps/web-shell/src/mocks/stacService.ts:464-474` is gone — both ends now use the same generated class). |
