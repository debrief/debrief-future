...
### 2026-06-01 - #266: Purge stale `bbox`/`zoom`/`center` SystemState-spatial references (spec-261 follow-up)
- **Status**: Implementation complete (PR pending)
- **Description**: Documentation-hygiene follow-up to #249/spec-261, which unified the `SystemStateProperties.spatial` shape onto `viewport: ViewportPolygon` (+ optional `rotation`), removing the legacy `bbox: float[4]` / `zoom: float` / `center: float[2]` fields. A repo-wide sweep found stale prose still describing the old shape in one forward-facing doc and two shipped specs; all were updated to the `ViewportPolygon` shape: `shared/schemas/README.md` (state.spatial variant line); `specs/237-active-storyboard-persistence/data-model.md` (SystemStateTypeEnum.spatial description); and `specs/022-system-kind-discriminator/` in full — the spec that originally *defined* the bbox/zoom/center shape — covering its acceptance criterion, Key-Entities prose, research rationale, data-model field table + validation rule, the Python usage example, and `evidence/sample-spatial.json` (rewritten to the 4-corner polygon and re-validated against the live `SystemState` Pydantic model). A "superseded by #261" note was added atop spec-022. Verified clean: the trigger greps (`SystemStateProperties.*bbox`, `state_type.*spatial.*bbox`, `(bbox, zoom[, center])`) return zero SystemState hits. Deliberately retained (different, current concepts): the storyboard `Viewport` type (`center`/`zoom`/`bearing`), STAC `spatial.bbox` collection extent, standard GeoJSON `bbox` members, and the new `ViewportPolygon.zoom` slot. No code, schema, or fixture changes — prose + one evidence fixture only.
- **Backlog**: BACKLOG.md item #266 (marked complete)

### 2026-06-02 - #256: Prefix-aware TypeScript typing for STAC extension properties
- **Status**: Implementation complete (PR #663)
- **Description**: Closes the #240-deferred Article II.1 finding — the STAC writers' modelled `debrief:*` access now derives from LinkML on both read and write. One schema-driven step in `generate.py` (`prefix_extension_slots()` + `_load_extension_slot_uri_map()`) rewrites modelled slot keys to their `slot_uri` across `StacExtensionProperties`, `StacSummaries`, and `StacAsset`. Modelled `debrief:toolId` + `debrief:snapshotTimestamp` as additive `StacAsset` slots (removes the `stacService.ts` hand-cast). Re-typed both hosts' write-path `props` from `Record<string,unknown>` to `StacItemProperties` (FR-012). Excluded `debrief:label` (a GeoJSON feature/annotation key, not a STAC property). Typing the `debrief:provenance_log` slot surfaced the #240 generated-vs-component-hybrid `PropertiesProvenanceEntry` divergence (previously masked by an `as`-cast); resolved with a typed persistence→domain narrowing bridge on the read. On-disk JSON unchanged; reuses the existing `src/generated` drift gate.
- **Scope note**: Expanded beyond the original spec during `/speckit.review` + `/speckit.plan` (user-directed): StacSummaries prefixing + StacAsset modelling folded into this PR rather than backlogged. Mechanism switched from text-only to schema-driven once the 3-class scope was confirmed.
- **ADR**: ADR-040 in `docs/project_notes/decisions.md`
- **Evidence**: `specs/256-prefix-aware-stac-typing/evidence/`
- **Backlog**: BACKLOG.md item #256

### 2026-08-07 - #285: Widen the ADR-reference guard from LinkML-only to the whole repository
- **Status**: Implementation complete (PR #678)
- **Description**: `scripts/check-adr-refs.sh`, wired into `task lint` by spec 205, scanned only `shared/schemas/src/linkml/*.yaml` — which contains exactly **2** ADR citations. The repository carries **588**, across `specs/` (111 files), `apps/` (23), `shared/` (21), `docs/` (11), `services/` (3), plus `CONSTITUTION.md`, `CLAUDE.md` and `STRATEGY.md`. Roughly 99.7% of ADR references were unvalidated, so the guard passed regardless of their state — a concern because `CLAUDE.md` directs every session to consult `decisions.md` before proposing architectural changes, and a citation resolving to nothing undermines that. Widened to the whole repo; added detection of malformed non-three-digit IDs (zero-padded four-digit forms look plausible in review but match no heading); switched from a working-tree walk to `git ls-files` so build output and uncommitted developer scratch cannot fail lint for everyone; failures now report `file:line` instead of a bare ID list. Runtime ~1.3s. Verified across four cases: clean repo passes; dangling ID in a tracked file fails; malformed four-digit ID fails separately; untracked scratch with bad IDs is ignored.
- **Finding**: `ADR-031` and `ADR-032` are cited from 12 files but have no `### ADR-NNN:` heading — the numbering note in `decisions.md` shows both were reserved for the #248/#249 extraction work and never written. Allowlisted in `RESERVED` with a pointer to that note rather than silently absorbed; whether to write them or retire the references is an open decision. No other dangling or malformed reference exists across all 588 citations.
- **Provenance**: Extracted from PR #677 (graphify adoption), closed unmerged. Graphify was trialled against the 20 most recent open issues and scored one clear win, two partial and seventeen no help — insufficient to justify 26MB of committed artefacts and four workflows. This guard was the one component that stood on its own merits and has no dependency on graphify.
- **Follow-ups**: decide the fate of `ADR-031`/`ADR-032`; separately, `python-lint.yml` runs unpinned `uvx ruff` while `task lint` runs lockfile-pinned `uv run ruff`, so CI and local can never agree (Article IX.2) — not fixed here.
- **Backlog**: BACKLOG.md item #285 (marked complete)

### 2026-09-14 - #286: UI designer requirements brief
- **Status**: Implementation complete (PR pending)
- **Description**: Wrote `docs/ui-designer-brief.md`, a self-contained UI & business-process requirements pack for an external UI designer commissioned to design the VS Code-based analyst UI. Covers the two UI modes (STAC Browser with no plot open; Plot Editor with control panels), four mandates (recognisably classic Debrief, native VS Code, modern practice, touch/tablet/phone), personas, VS Code conventions, ten business processes (ingest, find/resume, analyse, audit/revert/snapshot/branch, annotate, TMA, storyboard/brief, export, assistant, extend), the tool catalogue as a designer needs it (two category systems, closed-world selection matching, parameter types, four result kinds, legacy trigger mapping, drag tools needing touch twins), constitution-derived cross-cutting requirements, the still-open UI-review items, and a tensions list. 110 requirements (87 UI, 23 BP) with `UI-`/`BP-` IDs, MoSCoW priority and D/T/P device tags; traceability appendix maps every ID to its source docs, specs and ADRs.
- **Heritage**: classic-Debrief section derived from fifteen v3 user-guide screenshots supplied in-session (workbench, Time Controller, Track Tote, Outline, mouse modes, click buttons, Chart Overview, Properties, stacked dots, Chart Features, drawing toolbar, Grid Editor, tote semantics, time-variable plot, symbology). Indexed at `docs/design/legacy/README.md`; PNGs to be dropped in by the project lead. Keep/translate/drop table is marked as a proposal because no project document enumerates retired legacy views. Gaps surfaced: no v4 Tote, no Grid Editor, no platform symbology library, no chart furniture (scale/grid/on-map time), declared interaction modes lost since v3.
- **Decisions**: tablet = full analysis in VS Code-in-browser; phone = consumption-only web-shell PWA (ADR-041, reversing the April brief); deliverable = new doc superseding `docs/claude-design-ui-brief.md` (banner added); designer returns wireframes + interaction spec only.
- **ADR**: ADR-041 in `docs/project_notes/decisions.md`
- **Backlog**: BACKLOG.md item #286
