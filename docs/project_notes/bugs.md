...
- **2026-06-01** — Storyboard **Preview** button shows bare "Forbidden" under Heroku code-server (#273)
  ...
  - **Prevention:** Any loopback server opened via `asExternalUri` + `openExternal` must treat the rewritten host as legitimate, and must hand off sibling resources via **relative** URLs (proxy path-prefixes are invisible to the app). Covered by new unit tests in `apps/vscode/tests/unit/briefingPreviewServer.test.ts` (tunnel-host accepted; foreign host still 403) and `previewStoryboard.test.ts` (`trustExternalHost` called with the external URL). ADR-037 amended with the tunnel exception.
