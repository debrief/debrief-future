...
### Added
- **Unify `shared/components` bounds utilities with `@debrief/utils`** (#213 / spec 219) — Deletes the 215-line duplicate `shared/components/src/utils/bounds.ts` and consolidates all nine bounds helpers into `@debrief/utils`. Ports `expandBounds`, `isPointInBounds`, `bboxOverlapsViewport`, `viewportToBounds`, `filterBySpatialExtent` from `shared/components`. Adds pre-computed `feature.bbox` fast-path to `calculateBounds` (absorbs #211) — O(n features) map-fit latency for STAC collections. `BoundsInputFeature` structural minimum accepts `DebriefFeature[]`, `SafeFeature[]`, `GeoJSONFeature[]` without casts (Article XV). `shared/components` barrel re-exports unchanged — zero external consumer churn. ([#497](https://github.com/debrief/debrief-future/pull/497))
  - Tests: 300/300 `@debrief/utils`, 1647/1647 `@debrief/components`; `bounds.test.ts` grew from ~26 to 75 assertions (net +17, zero loss — SC-003); 5 compile-time type assertions in `bounds.types.test-d.ts`.
