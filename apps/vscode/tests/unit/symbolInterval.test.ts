 ...
 *
 * Validates that execute() correctly sets symbol_interval on the top-level
...
describe('symbolInterval (T034)', () => {
  it('basic golden example: sets symbol_interval on top-level properties', () => {
    const feature = makeTrackFeature();
    const result = execute([feature], { interval: 'PT30M' });

    expect(result).toHaveLength(1);
    expect(result[0].properties.symbol_interval).toBe('PT30M');
  ...
  it('does not modify default_position_style visibility', () => {
    const feature = makeTrackFeature();
    feature.properties.default_position_style = { show_symbol: false, symbol: 'diamond', show_label: true };

    const result = execute([feature], { interval: 'PT10M' });

    expect(result).toHaveLength(1);
    expect(result[0].properties.symbol_interval).toBe('PT10M');
    // Existing default_position_style must be completely preserved
    const dps = result[0].properties.default_position_style;
    expect(dps.show_symbol).toBe(false);
    expect(dps.symbol).toBe('diamond');
  ...
  it('overwrites existing interval: replaces prior symbol_interval value', () => {
    ...
    expect(result).toHaveLength(1);
    expect(result[0].properties.symbol_interval).toBe('PT1H');
  });
  ...
  it('default interval: uses PT15M when interval param is absent', () => {
    ...
    expect(result1[0].properties.symbol_interval).toBe('PT15M');

    const feature2 = makeTrackFeature();
    const result2 = execute([feature2], {} as Record<string, unknown>);
    expect(result2).toHaveLength(1);
    expect(result2[0].properties.symbol_interval).toBe('PT15M');
  });
});
