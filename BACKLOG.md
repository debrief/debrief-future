...
| 228 | Enhancement | Storyboard undo-stack capacity setting — expose the Storyboard edit undo buffer cap (hard-coded at 50/plot by #218) as a VS Code setting `debrief.storyboard.undoCapacity` (default 50, range 1–500). The 50-cap is our best guess against #215's scale bound; if real sessions show it too low (long polish runs hit cap mid-work) or too high (memory pressure on plots with deep provenance — worst case ~2.5 MB per plot per data-model.md §1), a setting lets us tune without a release. Low priority; revisit after telemetry. (requires #218) | 2 | 1 | 3 | 6 | Low | proposed |
