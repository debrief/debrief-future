...
| 200 | Tech Debt | [Consolidate bounds utilities into @debrief/utils](specs/200-bounds-consolidation/spec.md) — `apps/vscode/src/utils/bounds.ts` is a ~116-line 95%-identical copy of `shared/utils/src/bounds.ts`; vscode version has a null-guard the shared version is missing. Lift null-guard into utils, reconcile `SafeFeature`/`GeoJSONFeature` at the call site, delete the vscode copy + duplicate test, switch `mapPanel.ts` to import from `@debrief/utils`. | 3 | 1 | 5 | 9 | Low | implementing |
