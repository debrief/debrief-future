...
| ~~135~~ | ~~Bug~~ | ~~Log Panel webview fails to load in VS Code E2E (openvscode-server) — subsumed by #142, parked~~ | ~~4~~ | ~~1~~ | ~~4~~ | ~~9~~ | ~~Medium~~ | ~~parked~~ |
| ~~142~~ | ~~Infrastructure~~ | ~~[VS Code E2E webview reliability — research sprint](docs/ideas/142-vscode-e2e-webview-reliability.md) — parked; web-shell E2E covers components; hybrid approach (candidate F) adopted as pragmatic current strategy~~ | ~~5~~ | ~~3~~ | ~~2~~ | ~~10~~ | ~~High~~ | ~~parked~~ |
...
| 139 | Bug | [Document STAC ID naming convention](docs/ideas/139-document-stac-id-convention.md) — add ID convention (`[a-z0-9_-]` only) to STAC Extension spec from #125; add validation in `create_plot()` for custom IDs (BP-5) | 3 | 1 | 5 | 9 | Low | approved |
| 135 | Enhancement | [STAC link titles improvement](docs/ideas/135-stac-link-titles.md) — pass plot title to `_add_item_link()` for human-readable link titles; add titles to structural links (`root`, `parent`, `self`) in Items | 2 | 1 | 5 | 8 | Low | approved |
| 138 | Enhancement | [STAC `derived_from` links for provenance](docs/ideas/138-stac-derived-from-links.md) — add STAC-native `derived_from` link relation at Item level to complement existing `debrief:provenance` asset properties | 2 | 1 | 5 | 8 | Low | approved |
