...
| 137 | Enhancement | [REP loader temporal metadata](docs/ideas/137-rep-loader-temporal-metadata.md) — compute `start_datetime`/`end_datetime` from track position timestamps during REP file loading; enables accurate Timeline/Gantt view and Duration filter | 4 | 2 | 4 | 10 | Low | approved |
...
| 142 | Infrastructure | [VS Code E2E webview reliability — research sprint](docs/ideas/142-vscode-e2e-webview-reliability.md) — `resolveWebviewView` never fires in openvscode-server, causing ~15 test files (~50+ tests) to self-skip; research sprint to find reliable approach for real extension webview content in headless Playwright CI (subsumes #135) | 5 | 3 | 2 | 10 | High | approved |
