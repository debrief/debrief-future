...
| ~~210~~ | ~~Tech Debt~~ | ~~[Un-skip webview log-panel E2E suite](specs/210-unskip-log-panel-e2e/spec.md) — `describe.skip('Log Panel', … #143)` wrapper + stale blocker comment removed from `tests/e2e/test-log-panel.spec.ts`; the three tests now run through Playwright as individually `test.fixme(…, 'pending #509 …')` rather than silently dropped. FR-005 / research R7(b) escape hatch invoked on first live run: two deterministic failure modes isolated (LogPanel webview iframe not discoverable + `.leaflet-interactive` features never render); real work shifts to new blocker #509. Policy wins shipped (no stale `#143` ref, no suite-level skip, loud pending signal); SC-002 / SC-003 deferred to the feature that closes #509. (requires #143 [closed], #176 [merged]; blocks #509)~~ | ~~3~~ | ~~1~~ | ~~4~~ | ~~8~~ | ~~Low~~ | ~~complete~~ |
