...
| 205 | Tech Debt | [Promote DisplayMode and PlaybackState to LinkML](docs/ideas/205-displaymode-playbackstate-linkml.md) — two enum-style types are defined twice in TS with drifted values (`'full'\|'trail'` vs `'normal'\|'snailTrail'`; `'playing'\|'paused'` vs `'stopped'\|'playing'\|'paused'`). Define `DisplayModeEnum` + `PlaybackStateEnum` in LinkML, regenerate, delete both hand-typed copies per enum. Session-state's superset `PlaybackState` wins; components widens. | 4 | 2 | 4 | 10 | Medium | implementing |
