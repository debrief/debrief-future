...
| 203 | Tech Debt | [Consolidate spatial types in LinkML + lat/lon ↔ GeoJSON converters](specs/203-spatial-types-linkml/spec.md) — `Coordinate`, `ViewportPolygon`, `TimeFilter` defined twice in TS (components + session-state) using tuples, while LinkML has `Coordinate = {longitude, latitude}` object form. Adopt object form as canonical end to end; add `toGeoJSONCoord` / `fromGeoJSONCoord` helpers in `@debrief/utils` to confine tuple handling to GeoJSON boundary. | 4 | 2 | 4 | 10 | Medium | specified |
