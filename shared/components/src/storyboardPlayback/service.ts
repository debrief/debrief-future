/**
 * StoryboardPlaybackService — host-agnostic state machine for Feature 217
 ...
 *   - the air-gapped briefing renderer SPA (#264).
 *
 * Hoisted from `apps/vscode/src/services/storyboardPlayback.ts` during
 * the T-HOIST step of spec #264 so both consumers share one engine.
 *
...
import { runTimeRangeTween, type TimeRangeTweenHandle } from './timeRangeTween';
import type {
  SceneRowViewModel,
  StoryboardOptionViewModel,
  TransportViewModel,
  MissingDataReason,
} from '../panels/StoryboardPanel/types';
...
// ── Ports re-exported from ./ports ───────────────────────────────────
//
// `PlaybackPanelView` is declared inline below because its
// `applySnapshot` signature references `StoryboardPlaybackSnapshot`
// (declared in this file). The other ports are pulled in via the
// import block above and re-exported here for backwards compatibility
// with the original VS Code-side import sites.
export type {
  PlaybackMapPanel,
  PlaybackSessionManager,
  PlaybackTimeRangeView,
...
export class StoryboardPlaybackService implements HostDisposable {
  private readonly states = new Map<string, TransportState>();
  private readonly _onSnapshotChange = new HostEventEmitter<StoryboardPlaybackSnapshot>();
  public readonly onSnapshotChange: HostEvent<StoryboardPlaybackSnapshot> = this._onSnapshotChange.event;
  ...
  private readonly showInformationMessage: (message: string) => void;
  private readonly resolveThumbnailHref: (sceneId: string) => string;
  private readonly formatDtg: (iso: string) => string;

  private readonly rootDisposables: HostDisposable[] = [];
  ...
  constructor(options: StoryboardPlaybackServiceOptions) {
    ...
    this.showErrorMessage =
      ...
      ((msg): void => {
        // eslint-disable-next-line no-console -- fall-back when host doesn't wire the port.
        console.warn('[StoryboardPlaybackService] error:', msg);
      });
    this.setContextFn = options.setContext ?? ((): void => {});
  ...
  private classifyScene(
    ...
    const rawTimeRange = (temporal as { timeRange?: unknown } | undefined)?.timeRange;
    const timeRange =
      rawTimeRange &&
      typeof rawTimeRange === 'object' &&
      'start' in rawTimeRange &&
      'end' in rawTimeRange &&
      typeof (rawTimeRange as { start: unknown }).start === 'number' &&
      typeof (rawTimeRange as { end: unknown }).end === 'number'
        ? (rawTimeRange as { start: number; end: number })
        : null;
