 ...
 * ```
 */
export function computeHighlightSets(highlightedPaths: string[]): {
  ...
  for (const path of highlightedPaths) {
    // Add the path itself to direct paths
    directPaths.add(path);

    // Split path into segments and accumulate all ancestor paths
    const segments = path.split('/').filter(Boolean);
    let currentPath = '';

    for (let i = 0; i < segments.length - 1; i++) {
      currentPath = currentPath ? `${currentPath}/${segments[i]}` : `/${segments[i]}`;
      // Don't add to ancestorPaths if it's already in directPaths
      if (!directPaths.has(currentPath)) {
        ancestorPaths.add(currentPath);
      }
    }
  }

  return { directPaths, ancestorPaths };
}
