...
- TypeScript 5.x (for the loader source the config references); configuration itself is JSON (no runtime language). + `knip` (invoked ad-hoc via `pnpm dlx knip` — **not added as a project dependency**; keeping the zero-install status quo preserves Article IX). (201-knip-loader-config)
...
- 201-knip-loader-config: Added TypeScript 5.x (for the loader source the config references); configuration itself is JSON (no runtime language). + `knip` (invoked ad-hoc via `pnpm dlx knip` — **not added as a project dependency**; keeping the zero-install status quo preserves Article IX).
