...
- TypeScript 5.x (VS Code extension + webview + shared components) + VS Code Extension API ^1.85.0, React 18.x, `@debrief/components` (ChartRenderer), Zustand ^5.0.0 (`@debrief/session-state`), esbuild (webview bundling) (095-results-bottom-panel)
- Local filesystem STAC catalogs (read-only for this feature); result artifacts read from `assets/` sub-folders of STAC items (095-results-bottom-panel)
