...
- TypeScript 5.x (VS Code extension) + VS Code Extension API ^1.85.0, existing `stacService`, `sessionManager`, `recentPlotsService`, `MapPanel` (052-restore-plots-session)
- VS Code `workspaceState` (workspace-scoped key-value store, persisted by VS Code) (052-restore-plots-session)
