...
- TypeScript 5.x (strict mode, per Constitution XV) across `shared/components/src/nl-cql2/`, `apps/vscode/src/`, and `apps/nl-demo/`; Node.js ≥ 20 for the loopback proxy script (uses `node:http`, `node:https`). No Python changes. + Existing `shared/components` (nl-cql2 module — `LLMClient`, `providerCall`, `buildPrompt`, `parseResponse`); VS Code Extension API ^1.85.0 (`SecretStorage`, settings, postMessage); Node stdlib `node:http`/`node:https` for the proxy; browser-native `fetch` + `AbortController`; Anthropic Messages API (existing), OpenAI Chat Completions API (new — `/v1/chat/completions`), Ollama Chat API (new — `/api/chat`). (194-nl-search-providers)
...
## Recent Changes
- 194-nl-search-providers: Added TypeScript 5.x (strict mode, per Constitution XV) across `shared/components/src/nl-cql2/`, `apps/vscode/src/`, and `apps/nl-demo/`; Node.js ≥ 20 for the loopback proxy script (uses `node:http`, `node:https`). No Python changes. + Existing `shared/components` (nl-cql2 module — `LLMClient`, `providerCall`, `buildPrompt`, `parseResponse`); VS Code Extension API ^1.85.0 (`SecretStorage`, settings, postMessage); Node stdlib `node:http`/`node:https` for the proxy; browser-native `fetch` + `AbortController`; Anthropic Messages API (existing), OpenAI Chat Completions API (new — `/v1/chat/completions`), Ollama Chat API (new — `/api/chat`).
