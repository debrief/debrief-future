...
export function createLogService(deps: LogServiceDeps): LogService {
  return {
    ...
    async tuneEntry(
      ...
      // Before replaying, remove features that were generated by entries in
      // the replay plan. Without this, additive tools (e.g. generate-reference-
      // points) would create duplicates because the old generated features
      // remain on disk while new ones are appended.
      const generatedIdsToRemove = new Set<string>();
      for (const entry of plan.entries) {
        const timelineEntry = timeline.find((e) => e.activityId === entry.activityId);
        if (timelineEntry?.generated) {
          for (const gid of timelineEntry.generated) {
            generatedIdsToRemove.add(gid);
          }
        }
      }
      if (generatedIdsToRemove.size > 0) {
        ...
        if (preFc) {
          ...
          preFc.features = preFc.features.filter(
            (f) => !generatedIdsToRemove.has(fid(f as unknown as Record<string, unknown>))
          );
