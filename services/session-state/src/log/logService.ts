...
/**
 * Extract activityId from a raw provenance entry, handling both
 * camelCase (TypeScript) and snake_case (Python) key formats.
 */
function getEntryActivityId(entry: Record<string, unknown>): string | undefined {
...
export function createLogService(deps: LogServiceDeps): LogService {
  return {
    ...
    async tuneEntry(
      ...
      // Before replaying, remove features that were GENERATED (not used) by
      // entries in the replay plan. This prevents additive tools (e.g.
      // generate-reference-points) from creating duplicates.
      //
      ...
      const preFc = await deps.loadGeoJson(storePath, itemPath);
      if (preFc) {
        ...
        preFc.features = preFc.features.filter((f) => {
          const featureId = fid(f as unknown as Record<string, unknown>);
          // Never remove input features (they're used by the replay)
          if (usedFeatureIds.has(featureId)) { return true; }
          // Remove features whose provenance contains a replay-plan activityId
          const props = f.properties as Record<string, unknown> | null;
          const prov = props?.provenance;
          if (!Array.isArray(prov)) { return true; }
          for (const raw of prov) {
            const entry = raw as Record<string, unknown>;
            if (replayActivityIds.has(getEntryActivityId(entry) ?? '')) {
              return false; // generated by a replayed activity — remove it
            }
          }
          return true;
        });
        if (preFc.features.length < before) {
          await deps.writeGeoJson(
            storePath, itemPath,
            preFc as unknown as GeoJsonFeatureCollection
          );
        }
      }
      ...
      if (result.status === 'completed' && result.tuneAnnotation) {
        ...
        for (const feature of targetFc.features) {
          ...
          for (const raw of prov) {
            const entry = raw as Record<string, unknown>;
            if (getEntryActivityId(entry) === activityId) {
